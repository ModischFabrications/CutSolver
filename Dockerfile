...
RUN pip install --user --no-cache-dir --no-warn-script-location -r dev-requirements.txt certifi httpie
...
EXPOSE 80
HEALTHCHECK --interval=5m --timeout=10s CMD http --check-status http://localhost:80/ || exit 1
